        ...
        for i in range(count - 1, len(links)):
            ...
            if url.endswith(".pdf"):
                ...
                if "youtu" in url:
                    ...
                    try:
                        cc = f'* {str(count).zfill(3)}.* {name1}.zip \n*Batch »* {b_name}\n\n{creditx}'
                        Show = f"*⥥ Downloading ZIP »*\n\n*Name »* {name}\n\n*Url »* {url}"
                        prog = await m.reply_text(Show)
                        cmd = f'aria2c -o "{name}.zip" "{url}"'
                        os.system(cmd)
                        await prog.delete(True)
                        await bot.send_document(chat_id=m.chat.id, document=f'{name}.zip', caption=cc)
                        count += 1
